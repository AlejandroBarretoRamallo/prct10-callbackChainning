 ...
 * @param callback - Async function to be called after creating a directory and  a file
 */
export const checkDir = (user: string, funko: FunkoPop, callback: (error: string | undefined, funko?: FunkoPop, user?: string, filePath?: string) => void) => {
  const dirPath = path.join(process.cwd(), `/${user}`);
  const filePath = path.join(process.cwd(), `/${user}/${user}.json`);
  fs.access(dirPath, (err) => {
    if (err) {
      ...
      fs.mkdir(dirPath, { recursive: true }, (err) => {
        if (err) {
          log(chalk.red('Error creating directory'))
          callback('Error creating direcotry')
        }
        else {
          log(chalk.green('Directory created succesfully'))
          callback(undefined, funko, user, filePath)
        }
      });
    }
    else {
...
export const checkFile = (funko: FunkoPop, user: string, filePath: string, callback: (error: string | undefined, funko?:FunkoPop, user?: string) => void) => {
  fs.access(filePath, (err) => {
    if (err) {
      fs.writeFile(filePath, `[]`, (err) => {
        if (err) {
...
export const addCommand = {
  ...
  handler: (argv: ArgumentsCamelCase<AddFunko>) => {
    ...
    checkDir(argv.user, funko, (error, funko, user, filePath) => {
      if (!error) {
        checkFile(funko as FunkoPop, user as string, filePath as string, (error, funko, user) => {
          if (!error) {
            addFunko(funko as FunkoPop, user as string)
          }
        })
      }
    })
 ...
 * @param user - Username
 */
export const addFunko = (funko: FunkoPop, user: string) => {
